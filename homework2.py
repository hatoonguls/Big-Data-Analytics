...
Original file is located at
    https://colab.research.google.com/drive/1QD86dUcDj6nr373Zmesj4GsEnrm1DKVn
...
#business_cleaned_df['categories']
...
#business_cleaned_df['is_restaurant']

# TODO: Create individual rows for each category of each business (Hint: explode)
business_cleaned_df = business_cleaned_df.explode('categories').sort_values(by = ['business_id', 'categories']).reset_index(drop= True)
...
rest_df = business_cleaned_df[(business_cleaned_df['is_restaurant'] == 1) | (business_cleaned_df['categories'] == 'Restaurants')]
#rest_df

rest_df_biz = rest_df.groupby(['city', 'state', 'business_id'])['stars'].min().reset_index()
#rest_df_biz

#import numpy as np
rest_stars = rest_df_biz.groupby(['city', 'state']).agg(restaurant_count=('business_id','nunique'), average_stars=('stars','mean')).sort_values(by='restaurant_count', ascending=False).reset_index()
#rest_stars

top_ten = rest_stars.head(10)

most_res_df = rest_stars.sort_values(by='restaurant_count', ascending=False).head(10)

most_res_df.reset_index(drop=True, inplace=True)
...
#restaurants_df = business_cleaned_df[(business_cleaned_df['is_restaurant'] == 1) | (business_cleaned_df['categories'] == 'Restaurants')]
restaurants_df = business_cleaned_df[business_cleaned_df['categories'] == 'Restaurants']

checkin_bus_df = checkin_cleaned_df.groupby(['business_id'])['checkins'].sum().reset_index()

#res_check_df = restaurants_df.groupby(['business_id'])['checkins'].sum().reset_index()

#res_check_df

res_check = pd.merge(restaurants_df, checkin_bus_df, on='business_id', how='inner')
#res_check

#res_check_df = res_check.groupby(['city', 'state', 'business_id'])['checkins'].min().reset_index()

most_checkin_df = res_check.groupby(['city', 'state']).agg(avg_checkins=('checkins', 'mean')).sort_values(by='avg_checkins', ascending=False).head(15).reset_index()
#most_checkin_df

#most_checkin_df = most_checkin_df.sort_values(by='avg_checkins', ascending=False).head(15).reset_index(drop=True)
...
most_checkin_query = '''
SELECT b.city, b.state, AVG(c.checkins) as avg_checkins
FROM business_cleaned_df b
INNER JOIN checkin_bus_df c ON b.business_id = c.business_id
WHERE b.categories = 'Restaurants'
...
thai_restaurants = business_cleaned_df[business_cleaned_df['categories'].str.contains('Thai', case=False, na=False)]

max_reviews = thai_restaurants.groupby('state')['review_count'].max().reset_index()

popular_thai = pd.merge(thai_restaurants, max_reviews, on=['state', 'review_count'])

popular_thai.rename(columns={'review_count' : 'max_review'}, inplace=True)

popular_thai.sort_values(by=['max_review', 'name'], ascending=[False, True], inplace=True)

popular_thai_df = popular_thai.reset_index(drop=True)
...
popular_thai_query = '''
WITH ThaiRestaurants AS (
  SELECT *
  FROM business_cleaned_df
  WHERE categories LIKE '%Thai%'
),
MaxReviewsByState AS (
  SELECT state, MAX(review_count) as max_review
  FROM ThaiRestaurants
  GROUP BY state
)
SELECT a.state, a.city, a.name, b.max_review, a.stars
FROM ThaiRestaurants a
JOIN MaxReviewsByState b ON a.state = b.state AND a.review_count = b.max_review
ORDER BY b.max_review DESC, a.name ASC
'''
...
#Italian Restaurants in Phoenix, AZ
italian_res = business_cleaned_df[(business_cleaned_df['categories'].str.contains('Italian', case=False, na=False))]

italian_res_az = italian_res[(italian_res['city'] == 'Phoenix') & (italian_res['state'] == 'AZ')]
#italian_res_az

# TODO: Finding restaurants
best_italian_df = (italian_res_az[['business_id', 'name', 'stars', 'review_count', 'categories']].sort_values(by='review_count', ascending=False).head(10))

best_italian_df = best_italian_df.reset_index(drop=True)
...
#burgers restaurants
bur_res = business_cleaned_df[(business_cleaned_df['categories'].str.contains('Burgers', case=False, na=False))]

#burgers restaurants arizona
bur_res_az = bur_res[(bur_res['city'] == 'Phoenix') & (bur_res['state'] == 'AZ')].nlargest(5, 'review_count')

res_az_checkin = pd.merge(bur_res_az, checkin_cleaned_df, on='business_id', how='inner')

less_checkin = res_az_checkin[res_az_checkin['checkins'] < 5]

# TODO: Avoid Restaurant Traffic
best_burgers_df = less_checkin[['name', 'address', 'city', 'state', 'stars', 'review_count', 'categories', 'weekday', 'hour', 'checkins']].reset_index(drop=True)
#best_burgers_df
...
WITH TopBurgers AS (
  SELECT *
  FROM business_cleaned_df
  WHERE LOWER(categories) LIKE '%burgers%'
  AND city = 'Phoenix'
  AND state = 'AZ'
  ORDER BY review_count DESC
  LIMIT 5
...
SELECT b.name, b.address, b.city, b.state, b.stars, b.review_count, b.categories,
  c.weekday, c.hour, c.checkins
FROM TopBurgers b
INNER JOIN checkin_cleaned_df c ON b.business_id = c.business_id
WHERE c.checkins <5
...
filtered_users_df = user_cleaned_df[(user_cleaned_df['elite'] != 'None') & (user_cleaned_df['has_friend'] != 0) & (user_cleaned_df['review_count'] >= 2000)]

#user_cleaned_df

#user_cleaned_df['has_friend']

sorted_users_df = filtered_users_df.sort_values(by=['review_count', 'yelping_since'], ascending=[False, True])

sorted_users_df = sorted_users_df.reset_index(drop=True)

elite_user_df = sorted_users_df[['user_id', 'name', 'review_count', 'yelping_since', 'friends', 'elite', 'average_stars', 'has_friend']]
...
SELECT
  ...
  elite,
  average_stars,
  CASE WHEN friends != 'None' THEN 1 ELSE 0 END AS has_friend
FROM
  user_cleaned_df
WHERE
  elite != 'None' AND
...
elite_user_df['friends'] = elite_user_df['friends'].str.split(',').apply(lambda x: [friend.strip() for friend in x])
...
friends_merged_df = elite_user_df.merge(user_cleaned_df, left_on='friends', right_on='user_id', suffixes=('', '_friend'))
#friends_merged_df

friends_df = friends_merged_df[['user_id', 'name', 'average_stars', 'user_id_friend', 'name_friend', 'average_stars_friend']]

friends_df = friends_df.sort_values(by=['user_id', 'user_id_friend'])
...
SELECT
  ...
  a.name,
  a.average_stars,
  b.user_id AS user_id_friend,
  b.name AS name_friend,
  b.average_stars AS average_stars_friend
FROM
  elite_user_df a
JOIN
  user_cleaned_df b ON a.friends = b.user_id
ORDER BY
  a.user_id ASC,
  user_id_friend ASC
'''
...
friends_avg_stars = friends_df.groupby('user_id')['average_stars_friend'].mean().reset_index()

friends_avg_stars.rename(columns={'average_stars_friend' : 'avg_stars_friends'}, inplace=True)

merge_friends = pd.merge(friends_df[['user_id', 'average_stars']].drop_duplicates(), friends_avg_stars, on='user_id', how='left')

merge_friends['average_stars_diff'] = merge_friends['average_stars'] - merge_friends['avg_stars_friends']

stars_diff_df = merge_friends[['user_id', 'average_stars_diff']].drop_duplicates()
...
SELECT
  a.user_id,
  a.average_stars - AVG(a.average_stars_friend) AS average_stars_diff
FROM
  friends_df a
GROUP BY
  a.user_id, a.average_stars
ORDER BY
  a.user_id
'''
...
restaur_df = business_cleaned_df[business_cleaned_df['categories'].str.contains('Restaurants', case=False, na=False)]

restaur_tips = pd.merge(tip_cleaned_df, restaur_df, on='business_id', how='inner')

user_restaurant_tips = pd.merge(restaur_tips, user_cleaned_df, on='user_id', how='inner')

average_rating = user_restaurant_tips[['user_id', 'average_stars']].drop_duplicates().average_stars.mean()
...
WITH UserTips AS (
  ...
  a.user_id,
  b.average_stars
FROM tip_cleaned_df a
JOIN user_cleaned_df b ON a.user_id = b.user_id
JOIN business_cleaned_df c ON a.business_id = c.business_id
WHERE c.categories LIKE '%Restaurant%'
)

SELECT AVG(average_stars) AS average_stars
FROM UserTips
'''
...
rest_df1 = business_cleaned_df[business_cleaned_df['is_restaurant'] == 1]
#rest_df1

january_tips = tip_cleaned_df[tip_cleaned_df['date'].dt.month == 1]
#january_tips

tips_merged_df = january_tips.merge(rest_df1[['business_id', 'categories', 'review_count']], on='business_id', how='inner')
#tips_merged_df

cat_counts = tips_merged_df[['categories', 'user_id']].groupby('categories').count().reset_index()
#cat_counts

#cat_counts = cat_explode['categories'].value_counts()
#cat_counts.sum()

filtered_cat_counts = cat_counts[cat_counts['user_id'] >= 100]
#filtered_cat_counts

category_tip_df = filtered_cat_counts.reset_index(drop=True)
...
WITH JanuaryTips as (
  ...
  FROM tip_cleaned_df a INNER JOIN business_cleaned_df b ON a.business_id = b.business_id
  WHERE strftime('%m', a.date) = '01' and b.is_restaurant = 1
),
...
#checkin_cleaned_df['Time of Day'] = pd.to_datetime(checkin_cleaned_df['hour']).dt.strftime('%H:%M')#apply(lambda x: x.time())
checkin_cleaned_df['Time of Day'] = pd.to_datetime(checkin_cleaned_df['hour'], format = '%H:%M').dt.time
#pd.to_timedelta(checkin_cleaned_df['Time of Day']+':00')
checkin_cleaned_df.info()
#pd.to_datetime(df.TimeStamp).dt.strftime('%H:%M')
...
#pd.to_datetime(checkin_cleaned_df['hour'], format='%H:%M').apply(lambda x: x.time())
...
checkin_cleaned_df['Day'] = checkin_cleaned_df['weekday'].apply(lambda x: day_map[x])


checkin_cleaned_df = checkin_cleaned_df.drop('weekday', axis=1)
...
plot_df = checkin_cleaned_df[['business_id', 'checkins', 'Time of Day','Day']]
#plot_df
...
plot1_df = plot_df.groupby(['Day', 'Time of Day']).checkins.mean().reset_index()
plot1_df['xaxis'] = plot1_df['Time of Day'].apply(lambda x: x.strftime('%H:%M:%S'))
plot1_df['xaxis'] = plot1_df['xaxis'].apply(lambda x : int(x[:2])+1)
#print(plot1_df['xaxis'])

plot1_df.columns = ['day', 'Time of Day', 'Average Checkins', 'xaxis']
...
sns.lineplot(x='xaxis', y='Average Checkins', hue='day', data=plot1_df)
plt.xlabel("Time of Day")
plt.ylabel("Average Checkins")
plt.title("Average Checkins of Restaurants for each Hour of Day")
plt.xticks(ticks= plot1_df['xaxis'],labels = plot1_df['Time of Day'], rotation=45)
plt.tight_layout()
plt.legend(title='Day of the Week')
plt.show()

#plot1_df['Time of Day'].info()
...
#list(business_cleaned_df['categories'].unique())
business_american = business_cleaned_df[(business_cleaned_df['categories'] == 'American (New)') & (business_cleaned_df['is_restaurant'] == 1)]

bus_american_reviews = pd.merge(business_american, checkin_cleaned_df, how = 'inner', on = 'business_id')

bus_american_reviews = bus_american_reviews[['business_id', 'checkins', 'Time of Day', 'Day']]

#bus_american_reviews
...
plot2_df = bus_american_reviews.groupby(['Day', 'Time of Day']).checkins.mean().reset_index()
plot2_df['xaxis'] = plot2_df['Time of Day'].apply(lambda x: x.strftime('%H:%M:%S'))
plot2_df['xaxis'] = plot2_df['xaxis'].apply(lambda x : int(x[:2])+1)
#print(plot1_df['xaxis'])

plot2_df.columns = ['day', 'Time of Day', 'Average Checkins', 'xaxis']
...
plt.figure(figsize=(14, 7))
sns.lineplot(x='xaxis', y='Average Checkins', hue='day', data=plot2_df)
plt.xlabel("Time of Day")
plt.ylabel("Average Checkins")
plt.title("Average Checkins of Restaurants for each Hour of Day")
plt.xticks(ticks= plot2_df['xaxis'],labels = plot2_df['Time of Day'], rotation=45)
plt.tight_layout()
plt.legend(title='Day of the Week')
plt.show()
...
def tokenized_content(content):
  ...
  text_tokens = [word.lower() for word in tokens if (word.isalpha() and word.lower() not in stopwords)]
  return text_tokens
...
- From there, extract the `review` column of `hotels_reviews_df` as a list called `reviews`.
...
hotel_counts = Counter(hotels_df['Hotel_Name'])
#hotel_counts

most_reviewed_hotel = hotel_counts.most_common(1)[0][0]
#most_reviewed_hotel
...
top_tokens = [token for x in top_tokens_list for token in x]
...
top_most_common = token_counts.most_common(10)
...
#https://www.datacamp.com/tutorial/wordcloud-python referred to this link for wordcloud
